module AutoCompleteMacrosHelper      
  ...
  # muestra tambien el id del cliente
   def auto_complete_resultsucursal(entries, field, phrase = nil)
     items = []
     entries.each do |entry|
        items << content_tag("li", phrase ? highlight(entry.send field, phrase) : h(entry.send field) + " | #{entry.id[0]}" )
     end
     content_tag("ul", items.uniq)
  end
  # cuit, nombre y codigo proveedor
   def auto_complete_resultprovsucursal(entries, field, phrase = nil)
     items = []
     entries.each do |entry|
        items << content_tag("li", phrase ? highlight(entry.send field, phrase) : h(entry.send field) + " | #{entry.prov_nombre}" + " | #{entry.prov_codigo}" )
     end
     content_tag("ul", items.uniq)
  end

  # localidad, provincia, cod postal, id
  def auto_complete_resultlocalidadprov(entries, field, phrase = nil)
     items = []
     entries.each do |entry|
        items << content_tag("li", phrase ? highlight(entry.send field, phrase) : h(entry.send field) + " | #{entry.provincia.pcia_nombre}" + " |CP #{entry.loca_codigopostal}" + " | #{entry.id}")
     end
     content_tag("ul", items.uniq)
  end
